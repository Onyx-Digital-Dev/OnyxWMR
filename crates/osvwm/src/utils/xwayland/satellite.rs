...
pub fn setup(state: &mut State) {
    if state.osvwm.satellite.is_some() {
        return;
    }

    let config = state.osvwm.config.borrow();
    ...
    let event_loop = &state.osvwm.event_loop;
    ...
    state.osvwm.satellite = Some(Satellite {
        x11,
        abstract_token: None,
        unix_token: None,
        to_main,
...
fn setup_watch(state: &mut State) {
    let Some(satellite) = state.osvwm.satellite.as_mut() else {
        return;
    };
    ...
    if let Some(fd) = &satellite.x11.abstract_fd {
        ...
        let token = event_loop
            .insert_source(source, move |_, _, state| {
                if let Some(satellite) = &mut state.osvwm.satellite {
                    // Remove the other source.
                    if let Some(token) = satellite.unix_token.take() {
                        state.osvwm.event_loop.remove(token);
                    }
                    // Clear this source.
                    satellite.abstract_token = None;

                    debug!("connection to X11 abstract socket; spawning xwayland-satellite");
                    let path = state.osvwm.config.borrow().xwayland_satellite.path.clone();
