...
pub struct DisplayConfig {
    to_niri: calloop::channel::Sender<HashMap<String, Option<osvwm_config::Output>>>,
        ...
        for output in self.ipc_outputs.lock().unwrap().values() {
            ...
            if let Some(logical) = output.logical.as_ref() {
                let transform = match logical.transform {
                    osv_ipc::Transform::Normal => 0,
                    osv_ipc::Transform::_90 => 1,
                    osv_ipc::Transform::_180 => 2,
                    osv_ipc::Transform::_270 => 3,
                    osv_ipc::Transform::Flipped => 4,
                    osv_ipc::Transform::Flipped90 => 5,
                    osv_ipc::Transform::Flipped180 => 6,
                    osv_ipc::Transform::Flipped270 => 7,
                };
    ...
    async fn apply_monitors_config(
        ...
        for requested_config in logical_monitor_configs {
            ...
            for (connector, mode, _props) in requested_config.monitors {
                ...
                new_conf.insert(
                    connector.clone(),
                    Some(osvwm_config::Output {
                        off: false,
                        name: connector,
                        scale: Some(osvwm_config::FloatOrInt(requested_config.scale)),
                        transform: match requested_config.transform {
                            0 => osv_ipc::Transform::Normal,
                            1 => osv_ipc::Transform::_90,
                            2 => osv_ipc::Transform::_180,
                            3 => osv_ipc::Transform::_270,
                            4 => osv_ipc::Transform::Flipped,
                            5 => osv_ipc::Transform::Flipped90,
                            6 => osv_ipc::Transform::Flipped180,
                            7 => osv_ipc::Transform::Flipped270,
                            x => {
                                return Err(zbus::fdo::Error::Failed(format!(
                                    "Unknown transform {x}",
                                )))
                        ...
                        position: Some(osvwm_config::Position {
                            x: requested_config.x,
                            y: requested_config.y,
                        }),
                        mode: Some(osvwm_config::output::Mode {
                            custom: false,
                            mode: osv_ipc::ConfiguredMode::from_str(&mode).map_err(|e| {
...
// Adapted from Mutter.
fn make_display_name(output: &osv_ipc::Output, is_laptop_panel: bool) -> String {
    if is_laptop_panel {
