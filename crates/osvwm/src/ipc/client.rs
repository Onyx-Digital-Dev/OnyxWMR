...
pub fn handle_msg(mut msg: Msg, json: bool) -> anyhow::Result<()> {
    ...
    match msg {
        ...
        Msg::Layers => {
            ...
            let print = |surface: &osv_ipc::LayerSurface| {
                println!("    Surface:");
                println!("      Namespace: \"{}\"", &surface.namespace);

                let interactivity = match surface.keyboard_interactivity {
                    osv_ipc::LayerSurfaceKeyboardInteractivity::None => "none",
                    osv_ipc::LayerSurfaceKeyboardInteractivity::Exclusive => "exclusive",
                    osv_ipc::LayerSurfaceKeyboardInteractivity::OnDemand => "on-demand",
                };
                println!("      Keyboard interactivity: {interactivity}");
            };

            let print_layer = |iter: &mut Peekable<slice::Iter<osv_ipc::LayerSurface>>,
                               output: &str,
                               layer| {
                let mut empty = true;
            ...
            while let Some(surface) = iter.peek() {
                ...
                print_layer(&mut iter, output, osv_ipc::Layer::Background);

                print!("  Bottom layer:");
                print_layer(&mut iter, output, osv_ipc::Layer::Bottom);

                print!("  Top layer:");
                print_layer(&mut iter, output, osv_ipc::Layer::Top);

                print!("  Overlay layer:");
                print_layer(&mut iter, output, osv_ipc::Layer::Overlay);
            }
        }
        Msg::FocusedOutput => {
