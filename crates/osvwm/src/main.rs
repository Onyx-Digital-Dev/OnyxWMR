...
use osvwm::cli::{Cli, CompletionShell, Sub};
#[cfg(feature = "dbus")]
use osvwm::dbus;
use osvwm::ipc::client::handle_msg;
use osvwm::osvwm::State;
use osvwm::utils::spawning::{
...
use osvwm::utils::{cause_panic, version, watcher, xwayland, IS_SYSTEMD_SERVICE};
use osvwm_config::{Config, ConfigPath};
use osv_ipc::socket::SOCKET_PATH_ENV;
use portable_atomic::Ordering;
use sd_notify::NotifyState;
use smithay::reexports::wayland_server::Display;
use tracing_subscriber::EnvFilter;

const DEFAULT_LOG_FILTER: &str = "osvwm=debug,smithay::backend::renderer::gles=error";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.session {
        ...
        env::set_var("XDG_CURRENT_DESKTOP", "osvwm");
    ...
    osvwm::utils::signals::block_early().unwrap();

    // Avoid starting Tracy for the `osvwm msg` code path since starting/stopping Tracy is a bit
    // slow.
    tracy_client::Client::start();
    ...
    *CHILD_ENV.write().unwrap() = mem::take(&mut config.environment);
    ...
    osvwm::utils::signals::listen(&event_loop.handle());
    ...
    let socket_name = state.osvwm.socket_name.as_deref().unwrap();
    ...
    // Set OSVWM_SOCKET for children.
    if let Some(ipc) = &state.osvwm.ipc_server {
    ...
    if let Some(satellite) = &state.osvwm.satellite {
    ...
    if cli.session {
        ...
        #[cfg(feature = "dbus")]
        if !state.osvwm.config.borrow().input.disable_power_key_handling {
            if let Err(err) = state.osvwm.inhibit_power_key() {
    ...
    #[cfg(feature = "dbus")]
    if cli.session {
        state.osvwm.a11y.start();
    }

    if env::var_os("OSVWM_DISABLE_SYSTEM_MANAGER_NOTIFY").is_none_or(|x| x != "1") {
    ...
    if config_errored {
        state.osvwm.config_error_notification.show();
        state.ipc_config_loaded(true);
    } else if let Some(path) = config_created_at {
        state.osvwm.config_error_notification.show_created(path);
    }
...
fn env_config_path() -> Option<PathBuf> {
    env::var_os("OSVWM_CONFIG")
        .filter(|x| !x.is_empty())
        .map(PathBuf::from)
}

fn default_config_path() -> Option<PathBuf> {
    let Some(dirs) = ProjectDirs::from("", "", "osvwm") else {
...
fn system_config_path() -> PathBuf {
    PathBuf::from("/etc/osvwm/config.kdl")
}
