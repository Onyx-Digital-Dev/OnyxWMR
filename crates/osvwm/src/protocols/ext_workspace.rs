...
pub fn refresh(state: &mut State) {
    ...
    let protocol_state = &mut state.osvwm.ext_workspace_state;
    ...
    for (mon, _, ws) in state.osvwm.layout.workspaces() {
    ...
    protocol_state.workspace_groups.retain(|output, data| {
        if state.osvwm.sorted_outputs.contains(output) {
            return true;
        }
    ...
    for (mon, ws_idx, ws) in state.osvwm.layout.workspaces() {
    ...
    for output in &state.osvwm.sorted_outputs {
