...
use crate::osvwm::{CastTarget, PopupGrabState, State};
...
impl XdgShellHandler for State {
    fn xdg_shell_state(&mut self) -> &mut XdgShellState {
        &mut self.osvwm.xdg_shell_state
    }

    fn new_toplevel(&mut self, surface: ToplevelSurface) {
        let wl_surface = surface.wl_surface().clone();
        let unmapped = Unmapped::new(Window::new_wayland_window(surface));
        let existing = self.osvwm.unmapped_windows.insert(wl_surface, unmapped);
    ...
    fn move_request(&mut self, surface: ToplevelSurface, _seat: WlSeat, serial: Serial) {
        ...
        let Some((mapped, output)) = self.osvwm.layout.find_window_and_output(wl_surface) else {
            return;
        };
        ...
        match &start_data {
            ...
            PointerOrTouchStartData::Touch(_) => {
                let touch = self.osvwm.seat.get_touch().unwrap();
                if let Some(grab) = MoveGrab::new(self, start_data, window.clone(), true, None) {
    ...
    fn resize_request(
        ...
        let Some((mapped, _)) = self.osvwm.layout.find_window_and_output(wl_surface) else {
            return;
        };
    ...
    fn grab(&mut self, surface: PopupSurface, _seat: WlSeat, serial: Serial) {
        ...
        if self.osvwm.exit_confirm_dialog.is_open() {
            ...
            if Some(&root) != self.osvwm.lock_surface_focus().as_ref() {
            ...
            if let Some(layer) = layers.layer_for_surface(&root, WindowSurfaceType::TOPLEVEL) {
                ...
                if let Some(mapped) = self.osvwm.mapped_layer_surfaces.get(layer) {
                ...
                if layers.layers_on(Layer::Overlay).any(|l| {
                    (l.cached_state().keyboard_interactivity
                        ...
                        || Some(l) == self.osvwm.layer_shell_on_demand_focus.as_ref())
                        && self.osvwm.mapped_layer_surfaces.contains_key(l)
                }) {
                ...
                let layout_focus = self.osvwm.layout.focus();
        ...
        let can_receive_keyboard_focus = !self.osvwm.seat.input_method().keyboard_grabbed()
            && self
                .niri
                .layout
                .active_output()
        ...
        self.osvwm.popup_grab = Some(PopupGrabState {
            root,
            grab,
    ...
    fn maximize_request(&mut self, toplevel: ToplevelSurface) {
            ...
            .find_window_and_output_mut(toplevel.wl_surface())
        {
        ...
        } else if let Some(unmapped) = self.osvwm.unmapped_windows.get_mut(toplevel.wl_surface()) {
            match &mut unmapped.state {
                ...
                InitialConfigureState::Configured {
                    ...
                    let mon = output
                        .as_ref()
                        .and_then(|o| self.osvwm.layout.monitor_for_output(o))
                        .map(|mon| (mon, false))
                        // If not, check if we have a parent with a monitor.
                        .or_else(|| {
                            toplevel
                                .parent()
                                .and_then(|parent| self.osvwm.layout.find_window_and_output(&parent))
                    ...
                    *output = mon
                    ...
                    let ws = mon
                        ...
                        .or_else(|| self.osvwm.layout.active_workspace());
            ...
            match &mut unmapped.state {
                ...
                InitialConfigureState::Configured {
                    ...
                    let mon = workspace_name
                        .as_deref()
                        .and_then(|name| self.osvwm.layout.monitor_for_workspace(name))
                        .map(|mon| (mon, false));
                        ...
                        output
                            ...
                            .or_else(|| {
                                toplevel
                                    .parent()
                                    .and_then(|parent| {
                                        self.osvwm.layout.find_window_and_output(&parent)
                                    })
                                    .and_then(|(_win, output)| output)
                                    .and_then(|o| self.osvwm.layout.monitor_for_output(o))
                                    .map(|mon| (mon, true))
    ...
    fn toplevel_destroyed(&mut self, surface: ToplevelSurface) {
        ...
        self.backend.with_primary_renderer(|renderer| {
            self.osvwm.layout.store_unmap_snapshot(renderer, &window);
        });
        ...
        let active_window = self.osvwm.layout.focus().map(|m| &m.window);
        let was_active = active_window == Some(&window);

        self.osvwm.window_mru_ui.remove_window(id);
        self.osvwm.layout.remove_window(&window, transaction.clone());
        ...
        if !transaction.is_last() {
            transaction.register_deadline_timer(&self.osvwm.event_loop);
        }
    ...
    fn popup_destroyed(&mut self, surface: PopupSurface) {
        if let Some(output) = self.output_for_popup(&PopupKind::Xdg(surface)) {
            self.osvwm.queue_redraw(&output.clone());
        }
    }
    ...
    fn parent_changed(&mut self, toplevel: ToplevelSurface) {
        ...
        if let Some((mapped, output)) = self.osvwm.layout.find_window_and_output_mut(&parent) {
            let output = output.cloned();
            let window = mapped.window.clone();
            if self.osvwm.layout.descendants_added(&window) {
                if let Some(output) = output {
                    self.osvwm.queue_redraw(&output);
                }
            }
...
impl KdeDecorationHandler for State {
    fn kde_decoration_state(&self) -> &KdeDecorationState {
        &self.osvwm.kde_decoration_state
    }
...
impl XdgForeignHandler for State {
    fn xdg_foreign_state(&mut self) -> &mut XdgForeignState {
        &mut self.osvwm.xdg_foreign_state
    }
}
...
impl State {
    pub fn send_initial_configure(&mut self, toplevel: &ToplevelSurface) {
        ...
        let Some(unmapped) = self.osvwm.unmapped_windows.get_mut(toplevel.wl_surface()) else {
    ...
    pub fn queue_initial_configure(&self, toplevel: ToplevelSurface) {
        ...
        // initial commit.
        self.osvwm.event_loop.insert_idle(move |state| {
            if !toplevel.alive() {
                return;
            }

            if let Some(unmapped) = state.osvwm.unmapped_windows.get(toplevel.wl_surface()) {
    ...
    pub fn popups_handle_commit(&mut self, surface: &WlSurface) {
        self.osvwm.popups.commit(surface);

        if let Some(popup) = self.osvwm.popups.find_popup(surface) {
            match popup {
    ...
    pub fn output_for_popup(&self, popup: &PopupKind) -> Option<&Output> {
        ...
        self.osvwm.output_for_root(&root)
    }

    pub fn unconstrain_popup(&self, popup: &PopupKind) {
        let _span = tracy_client::span!("Osvwm::unconstrain_popup");
        ...
        } else if let Some((layer_surface, output)) = self.osvwm.layout.outputs().find_map(|o| {
    ...
    fn unconstrain_window_popup(&self, popup: &PopupKind, window: &Window) {
        ...
        let mut target = self.osvwm.layout.popup_target_rect(window);
    ...
    pub fn update_reactive_popups(&self, window: &Window) {
        let _span = tracy_client::span!("Osvwm::update_reactive_popups");
    ...
    pub fn update_window_rules(&mut self, toplevel: &ToplevelSurface) {
        ...
        if let Some(unmapped) = self.osvwm.unmapped_windows.get_mut(toplevel.wl_surface()) {
            ...
            .find_window_and_output_mut(toplevel.wl_surface())
        {
            if mapped.recompute_window_rules(window_rules, self.osvwm.is_at_startup) {
                drop(config);
                let output = output.cloned();
                let window = mapped.window.clone();
                self.osvwm.layout.update_window(&window, None);
...
pub fn add_mapped_toplevel_pre_commit_hook(toplevel: &ToplevelSurface) -> HookId {
    add_pre_commit_hook::<State, _>(toplevel.wl_surface(), move |state, _dh, surface| {
        ...
        let Some((mapped, _)) = state.osvwm.layout.find_window_and_output_mut(surface) else {
        ...
        if let Some(serial) = commit_serial {
            ...
            if let Some(transaction) = mapped.take_pending_transaction(serial) {
                ...
                let disable = state.osvwm.config.borrow().debug.disable_transactions;
                if !transaction.is_completed() && !disable {
                    ...
                    transaction.register_deadline_timer(&state.osvwm.event_loop);
                    ...
                    if !is_last {
                        ...
                        if let Some(client) = surface.client() {
                            transaction.add_notification(
                                state.osvwm.blocker_cleared_tx.clone(),
                                client.clone(),
                            );
            ...
            if let Some(client) = surface.client() {
                let res = state
                    ...
                    .insert_source(source, move |_, _, state| {
                        ...
                        let display_handle = state.osvwm.display_handle.clone();
                        state
        ...
        if got_unmapped {
            state.backend.with_primary_renderer(|renderer| {
                state.osvwm.layout.store_unmap_snapshot(renderer, &window);
            });
        } else {
            if animate {
                state.backend.with_primary_renderer(|renderer| {
            ...
            state.osvwm.layout.clear_unmap_snapshot(&window);
        }
    })
}
