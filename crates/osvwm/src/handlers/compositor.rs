...
use crate::osvwm::{CastTarget, ClientState, LockState, State};
...
impl CompositorHandler for State {
    fn compositor_state(&mut self) -> &mut CompositorState {
        &mut self.osvwm.compositor_state
    }
    ...
    fn new_subsurface(&mut self, surface: &WlSurface, parent: &WlSurface) {
        ...
        if let Some(output) = self.osvwm.output_for_root(&root) {
    ...
    fn commit(&mut self, surface: &WlSurface) {
        ...
        if surface == &root_surface {
            ...
            if let Entry::Occupied(entry) = self.osvwm.unmapped_windows.entry(surface.clone()) {
                if is_mapped(surface) {
                    ...
                    } = state
                    {
                        ...
                        let output =
                            output.filter(|o| self.osvwm.layout.monitor_for_output(o).is_some());

                        // Check that the workspace still exists.
                        let workspace_id = workspace_name
                            .as_deref()
                            .and_then(|n| self.osvwm.layout.find_workspace_by_name(n))
                            .map(|(_, ws)| ws.id());
                    ...
                    let output = self.osvwm.layout.add_window(
                        mapped,
                        target,
                        width,
                        height,
                    ...
                    if let Some((mapped, _)) = self.osvwm.layout.find_window_and_output(surface) {
                    ...
                    if let Some(output) = output {
                        self.osvwm.layout.start_open_animation_for_window(&window);

                        let new_focus = self.osvwm.layout.focus().map(|m| &m.window);
            ...
            if let Some((mapped, output)) = self.osvwm.layout.find_window_and_output(surface) {
                let window = mapped.window.clone();
                let output = output.cloned();
                ...
                if !is_mapped {
                    ...
                    self.osvwm.unmapped_windows.insert(surface.clone(), unmapped);
                ...
                self.osvwm.window_mru_ui.update_window(&self.osvwm.layout, id);
                self.osvwm.layout.update_window(&window, serial);

                // Move the toplevel according to the attach offset.
                if let Some(delta) = buffer_delta {
                    if delta.x != 0 || delta.y != 0 {
                        let (x, y) = delta.to_f64().into();
                        self.osvwm.layout.move_floating_window(
                            Some(&window),
        ...
        let root_window_output = self.osvwm.layout.find_window_and_output(&root_surface);
        if let Some((mapped, output)) = root_window_output {
            ...
            self.osvwm
                .window_mru_ui
                .update_window(&self.osvwm.layout, mapped.id());
        ...
        if matches!(
            &self.osvwm.cursor_manager.cursor_image(),
        ...
        if matches!(&self.osvwm.dnd_icon, Some(icon) if icon.surface == root_surface) {
            let dnd_icon = self.osvwm.dnd_icon.as_mut().unwrap();
        ...
        for (output, state) in &self.osvwm.output_state {
            if let Some(lock_surface) = &state.lock_surface {
                if lock_surface.wl_surface() == &root_surface {
                    if matches!(self.osvwm.lock_state, LockState::WaitingForSurfaces { .. }) {
                        self.osvwm.maybe_continue_to_locking();
                    } else {
                        self.osvwm.queue_redraw(&output.clone());
                    }
    ...
    fn destroyed(&mut self, surface: &WlSurface) {
        ...
        if let Some(root) = self.osvwm.root_surface.get(surface) {
        ...
        self.osvwm.dmabuf_pre_commit_hook.remove(surface);
    }
}
...
impl State {
    pub fn add_default_dmabuf_pre_commit_hook(&mut self, surface: &WlSurface) {
        ...
        let s = surface.clone();
        if let Some(prev) = self.osvwm.dmabuf_pre_commit_hook.insert(s, hook) {
    ...
    pub fn remove_default_dmabuf_pre_commit_hook(&mut self, surface: &WlSurface) {
        if let Some(hook) = self.osvwm.dmabuf_pre_commit_hook.remove(surface) {
