...
use osvwm_config::{CornerRadius, Gradient, GradientRelativeTo};
...
impl FocusRing {
    pub fn new(config: osvwm_config::FocusRing) -> Self {
        Self {
    ...
    pub fn update_config(&mut self, config: osvwm_config::FocusRing) {
        self.config = config;
    }
    ...
    pub fn config(&self) -> &osvwm_config::FocusRing {
        &self.config
    }
}
