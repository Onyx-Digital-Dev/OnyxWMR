//! Text rendering using fontdue with Inter font.
//!
//! Provides simple text rendering for the bar using the Inter font family.
...
const FONT_PATHS: &[&str] = &[
    ...
    // OSV bar resources (for bundled font)
...
impl TextRenderer {
    /// Create a new text renderer, searching for available fonts
    pub fn new() -> Option<Self> {
        ...
        tracing::warn!("No suitable font found, text rendering disabled");
        None
    }
    ...
    /// Render text centered horizontally within a region
    pub fn render_text_centered(
        ...
        let width = self.measure_text(text, size);
        let x = center_x - width / 2.0;
        self.render_text(pixmap, text, x, y, size, color);
    }

    /// Render text right-aligned
    pub fn render_text_right(
        &self,
        pixmap: &mut Pixmap,
        text: &str,
        right_x: f32,
...
/// Global text renderer instance (lazy initialized)
static TEXT_RENDERER: OnceLock<Option<TextRenderer>> = OnceLock::new();

/// Get the global text renderer (if available)
...
mod tests {
    ...
    #[test]
    fn test_font_paths_exist() {
        // At least verify the paths are valid strings
