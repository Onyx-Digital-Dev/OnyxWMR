//! Helper for blocking communication over the osvwm socket.
...
/// Name of the environment variable containing the osvwm IPC socket path.
pub const SOCKET_PATH_ENV: &str = "OSVWM_SOCKET";

/// Helper for blocking communication over the osvwm socket.
///
/// This struct is used to communicate with the osvwm IPC server. It handles the socket connection
...
impl Socket {
    /// Connects to the default osvwm IPC socket.
    ///
    ...
    pub fn connect() -> io::Result<Self> {
        let socket_path = env::var_os(SOCKET_PATH_ENV).ok_or_else(|| {
            io::Error::new(
                ...
                format!("{SOCKET_PATH_ENV} is not set, are you running this within osvwm?"),
            )
        })?;
        Self::connect_to(socket_path)
    }

    /// Connects to the osvwm IPC socket at the given path.
    ...
    /// Sends a request to osvwm and returns the response.
    ///
    /// Return values:
    ///
    /// * `Ok(Ok(response))`: successful [`Response`](crate::Response) from osvwm
    /// * `Ok(Err(message))`: error message from osvwm
    /// * `Err(error)`: error communicating with osvwm
