...
//!
//! This crate follows the osvwm version. It is **not** API-stable in terms of the Rust semver. In
...
// Variants in this enum should match the spelling of the ones in osvwm-config. Most, but not all,
// variants from osvwm-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
...
// Variants in this enum should match the spelling of the ones in osvwm-config. Most thigs from
// osvwm-config should be present here.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[cfg_attr(feature = "clap", derive(clap::Parser))]
#[cfg_attr(feature = "clap", command(subcommand_value_name = "ACTION"))]
#[cfg_attr(feature = "clap", command(subcommand_help_heading = "Actions"))]
...
pub enum OutputAction {
    ...
    Mode {
        ...
        ///
        /// Run `osvwm msg outputs` to see the available modes.
        #[cfg_attr(feature = "clap", arg())]
        mode: ModeToSet,
    },
...
/// some optional properties are present or absent for certain window types may change across osvwm
/// releases.
///
...
pub struct WindowLayout {
    ...
    ///
    /// Does not include osvwm decorations like borders.
    ///
    ...
    ///
    /// This is the same "workspace view" as in gradients' `relative-to` in the osvwm config.
...
pub struct Workspace {
    ...
    ///
    /// This is the same index you can use for requests like `osvwm msg action focus-workspace`.
    ///
